  ...
  mutate(
   ...
   # ),
    ...
    only_device3 = case_when(
      comp_tab == 1 & int_acc == F & smrtphone == F ~ 1,
    ...
    smrt_and_internet4 = case_when(
      ...
      comp_tab == 1 | int_acc == F | smrtphone == F ~ 0,
      TRUE ~ NA_real_),
    ...
    #no device or internet but yes smartphone (#6)
     only_smartphone6 = case_when(
      ...
      int_acc == T | comp_tab == 1 | smrtphone == F ~ 0,
      TRUE ~ NA_real_),
    ...
    device_smrt_only8 = case_when(
      ...
      int_acc == T | comp_tab == 0 | smrtphone == F ~ 0,
      TRUE ~ NA_real_),
    total = internet_and_device + nohiinternet_and_device2 + only_device3 + smrt_and_internet4 +
...
variables <- list("int_acc","hspd_int", "regular_internet","comp_tab", "comp_tab_smrt",
                  "internet_and_device","nohiinternet_and_device2",
                  "only_device3","smrt_and_internet4","only_internet5","only_smartphone6",
                  "nothing7","device_smrt_only8")

#variables2 <- list("int_acc","hspd_int", "regular_internet","comp_tab", "comp_tab_smrt",
                  "access_type")
                 
#make county dataframe with all the variables 
for(x in variables) {
  df <- survey_by_demog(acs_micro_internet, x)
  digitalaccess_county <- assign_col_join(digitalaccess_county, df)
  rm(df)
}

#what needs to be null for 2013-2016
smrt_and_internet4, only_smartphone6, device_smrt_only8, nothing

percentdigitalaccess_county <- digitalaccess_county %>% 
  filter(var_type=="percent") %>% mutate(
  var_type = NULL)

#now make dataframe that makes column per category

digitalaccess_groupings_county <- survey_by_demog(acs_micro_internet, "access_type")

#compare
test <- digitalaccess_county %>% filter(year > 2015)
test2 <- test %>% select(FIPS, year, sex, race, var_type,only_device3)
test2 %<>% rename(CaseA_onlydevice = only_device3)
testaccess <- digitalaccess_groupings_county %>% select(FIPS, year, sex, race, var_type,only_device3)
testaccess %<>% rename(CaseB_onlydevice = only_device3)

error <- assign_col_join(test2, testaccess)

#filter down to correct years and make total column to check (this works, so omitting it for now but can always go back and check)
digitalaccess_groupings_county %<>%
  filter(year > 2015) #%>%
 # mutate(total = internet_and_device1 + nohiinternet_and_device2 + only_device3 + smrt_and_internet4 +
   # only_internet5 + only_smartphone6 + nothing7 + device_smrt_only8)

#make MSA dataframe with all the variables 
for(x in variables) {
  df <- survey_by_demog(acs_micro_internet, x, geog="MSA")
